...
§
Mihomo proxy installed and configured at /etc/mihomo/. Config: /etc/mihomo/config.yaml, subscription URL from yfssce.net. Mixed proxy on 127.0.0.1:7890, SOCKS on 7891, API on 9090. Systemd service: mihomo.service (enabled, running). Download geoip/geosite from ghproxy.net mirror. Use export https_proxy=http://127.0.0.1:7890 http_proxy=http://127.0.0.1:7890 for terminal proxy. Config has emoji-named proxy groups with China direct + foreign proxy rules.
§
All foreign/international downloads and requests should go through the mihomo proxy (http://127.0.0.1:7890). Use export https_proxy/http_proxy before any curl/wget/git clone to foreign sources. Domestic sources (aliyun, tencent, pypi mirrors, etc.) can stay direct.
§
